<template>
	<view class="complete">
		<!-- 已完成类型订单 -->
		<view class="dingdan" v-for="(item,index) in completeList" :key="index">
			<!-- 订单号 -->
			<view class="state">
				<text class="orderNumber">订单号：DK-{{item.Id}}</text>
				<text class="red">已完成</text>
			</view>
			<!-- 订单明细 -->
			<view class="main" v-for="item1 in item.item">
		...
		<!-- 没有订单 显示此图片 -->
		<image v-if="!completeList.length" class="wudingdan" src="../../../static/home/wu.png" mode=""></image>
	</view>
</template>
