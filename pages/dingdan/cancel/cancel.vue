<template>
	<view class="cancel">
		<!-- 已取消类型订单 -->
		<view class="dingdan" v-for="(item,index) in cancelList" :key="index">
			<!-- 订单号 -->
			<view class="state">
				<text class="orderNumber">订单号：DK-{{item.Id}}</text>
				<text class="red">已取消</text>
			</view>
			<!-- 订单明细 -->
		...
		<!-- 没有订单 显示此图片 -->
		<image v-if="!cancelList.length" class="wudingdan" src="../../../static/home/wu.png" mode=""></image>
	</view>
</template>
